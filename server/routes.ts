...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/mile-tracker/sessions", requireFeatureAccess(PREMIUM_FEATURES.MILE_TRACKER), async (req, res) => {
    try {
      const userId = getAuthUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app.get("/api/mile-tracker/sessions/active", requireFeatureAccess(PREMIUM_FEATURES.MILE_TRACKER), async (req, res) => {
    try {
      const userId = getAuthUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
  ...
  app.post("/api/mile-tracker/sessions", requireFeatureAccess(PREMIUM_FEATURES.MILE_TRACKER), async (req, res) => {
    try {
      const userId = getAuthUserId(req);
      if (!userId) {
        return res.status(401).json({ message: "User not authenticated" });
